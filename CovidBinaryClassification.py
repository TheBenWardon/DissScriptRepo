# -*- coding: utf-8 -*-
"""
Created on Sun Mar  6 18:57:52 2022
...
ModelName = "CovidClassification_MFCC_JustCOVID-{}".format(int(time.time()))
...
def CreateMFCC(Directory):
    ...
    ShapeString = str(mfcc.shape) #mfcc #SpecGram
...
def CreateFinalDataSets(): 
    ...
    TheOverallDataset = HealthyDataset.concatenate(NotHealthyDataset)
...
if __name__ == "__main__":
    ...
    #             if row["status"] == "COVID-19":
    #                 DirectoryToSave = NotHealthyFilePath + Cunt + str(FileName) +  "_1_"
    #             elif row["status"] == "symptomatic":
    #                 DirectoryToSave = NotHealthyFilePath + Cunt + str(FileName) +  "_0_"
    #             elif row["status"] == "healthy":
    ...
    MyModel.compile(
        loss = tf.keras.losses.BinaryCrossentropy(from_logits=False), #SparseCategoricalCrossentropy BinaryCrossentropy
        optimizer="adam",
        metrics="accuracy"
        )
    MyModel.fit(TheTrainingDataset, validation_data=TheValidationDataset, batch_size=BatchSize, epochs=32, callbacks=MyTensorBoard)
    
    TestLoss, TestAccuracy = MyModel.evaluate(TheTestingDataset, batch_size=BatchSize, verbose = 2)
    print(TestLoss, TestAccuracy)
    
    MyModel.save(r"E:\Python\Diss\Models\CovidClassification_MFCC_JustCOVID")
